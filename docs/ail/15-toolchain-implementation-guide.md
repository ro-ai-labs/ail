...
trace and resuming the caller after the callee's final `RETURN_SUCCESS`.
Native code also supports straight-line `ADD_INT_FIELD` integer updates,
writing the updated `key=value` state line to stdout and the matching VM-style
add trace to stderr; missing or non-integer runtime fields exit nonzero.
Unsupported loops and future VM opcodes still fail native compilation rather
than emitting partial executables. Stdout remains reserved for parseable state
changes. The first native backend rejects future unknown VM opcodes and unlowered
...
malformed target-support statuses and static operands such as a non-integer
`ADD_INT_FIELD.delta` before runtime state is touched.
...
`bootstrap-pass-order-diagnostics.txt`, `bootstrap-native-bytecode-report.txt`,
...
AIL-Meta `InferReadPermissions` compiler pass through the Linux syscall argv
ABI, then records that handoff evidence in a fingerprinted report. The manifest
records
...
report, pass-order diagnostics report, native-bytecode report,
host-boundary report, dependency report, native-handoff report, bytecode,
...
checked-core fingerprint, compiler-pass trace, fixed-point report fingerprint,
pass-composition fingerprint, pass-order diagnostics fingerprint,
native-bytecode report fingerprint, conformance report fingerprint,
host-boundary report fingerprint, dependency report fingerprint,
native-handoff report fingerprint, bytecode fingerprints, and native target
fingerprints before the bundle is accepted. The dependency report
...
`ail-interview` runs the first AI-IDE conversation step with
`docs/ail/prompts/interview.system.md`. It asks the package base LLM for an
...
`declare_provenance` graph operations, runs the AIL-Core checker inside the
patch applier, and prints the patched Core artifact only when the checker
accepts the result. The patch `base_hash` must match the canonical checked Core
hash before any operation is applied. If the patch includes `package`, it must
be a string and must match the checked Core package name before any operation
is applied. AIL-Flow exposes the package name as `package`, the hash as
`coreHash`, node patch labels as `coreLabel`, and patch-ready edge labels as
`edgeRefs` on action, tool, compiler-pass, and system-component projections.
Node additions reject existing kind and name pairs. Node removals are guarded:
they only delete detached nodes and require relationships to be removed first.
Edge additions reject existing source, target, and kind triples, so an agent
must use `replace_edge_attributes` for attribute changes. Attribute replacement
is key-merge based; node edits can update `type` and rewire changed stable node
