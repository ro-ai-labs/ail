...
`bootstrap-host-boundary-report.txt`, and `manifest.ail-bootstrap.txt`. The
bootstrap command runs the AIL-Meta compiler
...
report, native-bytecode report, host-boundary report, bytecode, conformance
reports, and native executable bytes. The AIL-authored
...
report fingerprint, host-boundary report fingerprint, bytecode fingerprints,
and native target fingerprints before the bundle is accepted, so the bootstrap
boundary is reviewable as AIL source, checked IR, stable AIL compiler-pass
output, AIL bytecode, reports, and machine-level Linux ELF artifacts rather
