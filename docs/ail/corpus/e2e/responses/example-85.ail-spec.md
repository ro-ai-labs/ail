...
compress2 records trace event named ForeignCallCompress2Scenario085
...
qsort records trace event named ForeignCallbackComparedScenario085
...
- PacketHeaderLayoutCheckedScenario085

Action: Compress payload.

When compress payload happens:

- the system records a trace event named PayloadCompressedScenario085

Failure OutOfMemory happens when zlib reports memory exhaustion:

- the caller sees "Out of memory"
- the trace records ForeignOutOfMemoryScenario085
...
- the trace records ForeignOutputBufferTooSmallScenario085

Failure InvalidComparator happens when libc rejects the callback comparator:

- the caller sees "Invalid comparator"
- the trace records ForeignInvalidComparatorScenario085
