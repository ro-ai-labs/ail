...
- Counted catalog examples: 128
- Accepted prompt-to-artifact examples: 119
- Rejected diagnostic examples: 9
- Story files: 128
- Stored request transcripts: 128
- Stored response artifacts: 128
- Rejected repair tutorials: 9 generated by replay and fingerprinted in the
  artifact manifest
- Rejected repair proof chains: 9 corrected specs, 9 checked Core artifacts, 9
  verified bytecode artifacts, 6 VM traces, and 3 target reports generated by
  replay
- Rejected repair diffs: 9 generated by replay and fingerprinted in the
  artifact manifest
- Rejected repair promotion reviews: 9 generated by replay and fingerprinted in
...
bytecode, and runtime or target evidence with manifest fingerprints. Replay
also writes `repair-diff.txt` so reviewers can compare rejected and repaired
fingerprints, confirm the expected diagnostic is removed, and inspect preserved
story anchors. Replay writes `repair-promotion-review.txt` to bind that repair
evidence to an explicit `accepted-for-promotion` proposal that still requires
human approval before any corpus promotion.
...
| `examples/incident_response.ail/` | `incident-response` | Application | 7 | High-level multi-module incident workflows, UI surfaces, target contracts, regenerated story views, and two promoted package-local repair proofs. |
...
| `examples/repeated_task.ail/` | `repeated-task` | Application | 6 | Scheduled maintenance workflow, repeated action lowering, VM trace, temporal-policy fixtures, `AIL-WORKFLOW-001` diagnostics, and a promoted User Story mode scheduler example. |
...
| `examples/ui_workflow.ail/` | `ui-workflow` | UI | 5 | UI route, form, dashboard, workflow ordering, accessibility, rejected inaccessible-error diagnostics, Wasm target-contract evidence, and promoted User Story mode workflow evidence. |
...
components are not allowed. Replay also checks package closure: every
top-level `examples/*.ail` directory must either appear as a counted
`package:` in `examples/examples.md` or be declared support-only in
`examples/support-packages.md` with a concrete `used-by` relationship.
...
- `examples/support-packages.md` when the package is support-only or stops
  being support-only
- this inventory
...
list from disk and rejects inventory or support-manifest drift.
