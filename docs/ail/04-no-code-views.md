...
operations. The patch must include `base_hash` for the canonical checked
AIL-Core artifact; stale patches are rejected before any operation runs. The
CLI then runs the AIL-Core checker before printing the patched Core artifact.
Attribute edits rewire changed stable node ids before checking, so existing
rules, traces, failures, and provenance stay attached to the edited node. The
patched Core can be rendered back to AIL-Spec with `ail-spec --core-file`.
