...
harness review, plan-only capture artifact, import-demo evidence, optional
hosted Story Promotion live reviewer artifacts such as
`story-promotion-live-review-report.txt`,
`story-promotion-live-review-review.txt`, and
`manifest.v03-story-promotion-live-review.txt`, visible token-budget checks, and
...
It also requires `story-promotion-batch-plan.txt` and
`manifest.v03-story-promotion-batch-plan.txt` before the promoted stories are
claimed as one reviewed batch.
...
`repair-promotion` manual chapter evidence, the
`rejected-repair-audit-report.txt` batch audit, plus the plan-only capture
artifact and import-demo evidence required before a repaired rejected example
