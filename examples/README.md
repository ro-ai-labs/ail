...
anchors forward. That makes diagnostic examples end-to-end authoring loops
instead of prose-only negative cases.
...
repair-bytecode, repair-vm-trace, repair-target-report, and repair-diff
fingerprint reuse. Response, extracted-artifact, and target-report duplicate
counts must remain zero before claiming the v0.2 prompt-to-artifact release
gate. Rejected example replay
...
semantic-anchor preservation. All repair artifacts are listed in
