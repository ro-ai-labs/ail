...
`distinctness-claim`, and `v0.3-signal` metadata so the verifier can prove that
the catalog spans useful work from low-level development to high-level
workflows.
...
- `examples.md`: 111 manifest entries with prompt, executor, profile, surface,
  use-case, capability-level, capability-under-test, distinctness,
  capture-origin, checker-result, target, and v0.3 learning metadata. One
  hundred three entries are accepted prompt-to-artifact examples that replay
  through checked Core, bytecode, VM trace, and binary or target-contract
  evidence; eight entries are rejected diagnostic examples.
...
capability-level counts, capture-origin counts, response,
