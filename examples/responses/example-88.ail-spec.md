...
compress2 records trace event named ForeignCallCompress2Scenario088
...
qsort records trace event named ForeignCallbackComparedScenario088
...
- PacketHeaderLayoutCheckedScenario088

Action: Compress payload.

When compress payload happens:

- the system records a trace event named PayloadCompressedScenario088

Failure OutOfMemory happens when zlib reports memory exhaustion:

- the caller sees "Out of memory"
- the trace records ForeignOutOfMemoryScenario088
...
- the trace records ForeignOutputBufferTooSmallScenario088

Failure InvalidComparator happens when libc rejects the callback comparator:

- the caller sees "Invalid comparator"
- the trace records ForeignInvalidComparatorScenario088
