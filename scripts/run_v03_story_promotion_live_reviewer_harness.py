#!/usr/bin/env python3
"""Run the AIL v0.3 Story Promotion live reviewer harness.

This harness is opt-in. It asks the story-promotion reviewer role to inspect
the deterministic User Story mode promotion evidence and records the hosted
request/response bundle for offline review before any promoted story enters the
checked examples corpus.
"""
...
DEFAULT_ARTIFACT_DIR = "/tmp/ail-v03-story-promotion-live-review"
DEFAULT_EXAMPLES_ARTIFACTS = "/tmp/ail-manual-story-promotion-examples"
DEFAULT_STORY_ARTIFACTS = "/tmp/ail-v03-story-llm"
DEFAULT_CAPTURE_PLAN_DIR = "/tmp/ail-v03-story-promotion-capture-plan"
DEFAULT_IMPORT_WORK_DIR = "/tmp/ail-v03-story-promotion-import-work"
...
ROLE_CONTRACTS = (
    ("story-promotion-reviewer", "examples/agents/codex-ail-story-promotion-reviewer.md"),
)
SKIPPED_MODEL_CHECK_ERROR = (
    "model-check skipped; hosted Story Promotion reviewer evidence requires "
...
def check_fingerprint(
    ...
    fingerprint_path = fingerprint_path or fingerprint_path_for(path)
...
def optional_release_summary_paths(args: argparse.Namespace) -> list[tuple[str, Path]]:
    return [("examples-report.txt", Path(args.examples_artifacts) / "examples-report.txt")]
...
def evidence_paths(args: argparse.Namespace) -> list[tuple[str, Path]]:
    return [
        (
            ...
            / "examples"
            / args.proposed_entry_id
            / "story-promotion-review.txt",
        ),
        (
            "story-llm-harness-report.txt",
            Path(args.story_artifacts) / "story-llm-harness-report.txt",
        ),
        (
            "story-promotion-capture-plan.json",
            Path(args.capture_plan_dir) / "story-promotion-capture-plan.json",
        ),
        (
            "story-promotion-import-demo-report.txt",
            Path(args.import_work_dir) / "story-promotion-import-demo-report.txt",
        ),
    ]
        ...
        summary_sections.append(
            "\n".join(
                [
                    ...
                    *(f"required-snippet {line}" for line in required_snippet_lines),
                    "content-excerpt:",
...
{{
  ...
  "role": "{role}",
  "decision": "accept",
  "evidence": ["story-promotion-review.txt"],
  "questions": [],
  "checker_handoff": {{
    "must_check": true,
    "required_artifacts": ["story-promotion-review.txt"]
  }}
}}
...
- evidence must name the story promotion artifacts reviewed.
- checker_handoff.must_check must be true.
- Return accept only if the story LLM harness report, deterministic promotion
  review, capture plan, and import demo report are all present and coherent.
"""
...
def role_probe(
    ...
    return f"""Review User Story mode promotion evidence.
...
Determine whether your role accepts the promotion for human-approved import.
...
def classify_reviewer_content(content: str, role: str) -> tuple[str, str, str]:
    ...
    if envelope.get("artifact_kind") != ARTIFACT_KIND:
        return "invalid", "missing", f"reviewer artifact_kind must be {ARTIFACT_KIND}"
    ...
    if decision not in {"accept", "needs-repair", "reject"}:
        return (
            "invalid",
            "missing",
            "reviewer decision must be accept, needs-repair, or reject",
        )
...
def nonaccept_backlog_entry(role: str, content: str, decision: str) -> list[str]:
    ...
    envelope, error = parse_envelope(content)
    if envelope is None:
        return [f"role {role} decision {decision}", f"role {role} envelope-error {error}"]
    evidence = envelope.get("evidence", [])
...
def render_nonaccept_backlog(
    ...
    lines = [
        "AIL-Story-Promotion-Live-Review-Repair-Backlog:",
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run live Story Promotion reviewer probes. Use --dry-run to list "
            "the reviewer probe without network access."
        ),
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "examples/<proposed-entry-id>/story-promotion-review.txt "
            f"(default: {DEFAULT_EXAMPLES_ARTIFACTS})"
        ),
    )
    parser.add_argument(
        "--story-artifacts",
        default=DEFAULT_STORY_ARTIFACTS,
        help=(
            "Story LLM artifact directory containing story-llm-harness-report.txt "
            f"(default: {DEFAULT_STORY_ARTIFACTS})"
        ),
    )
    parser.add_argument(
        "--capture-plan-dir",
        ...
        help=(
            "Directory containing story-promotion-capture-plan.json "
    ...
    parser.add_argument(
        ...
        help=(
            "Directory containing story-promotion-import-demo-report.txt "
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print reviewer probe without contacting the LLM endpoint",
    )
    parser.add_argument(
        "--skip-model-check",
        action="store_true",
    ...
    parser.add_argument(
        "--review-artifacts",
        help="Review an existing Story Promotion live reviewer artifact directory",
    )
    parser.add_argument(
...
def token_budget_lines(max_tokens: int) -> list[str]:
    if max_tokens < DEFAULT_MAX_TOKENS:
        return ["token-budget-default false", "token-budget-warning max-tokens-below-default"]
    if max_tokens == DEFAULT_MAX_TOKENS:
        return ["token-budget-default true"]
    return ["token-budget-default false", "token-budget-warning max-tokens-above-default"]
...
def print_dry_run(args: argparse.Namespace, contracts: list[tuple[str, str, str]]) -> None:
    ...
    print("AIL-Story-Promotion-Live-Reviewer-Harness:")
    ...
    print(f"story-artifacts {args.story_artifacts}")
...
def review_artifacts(args: argparse.Namespace, contracts: list[tuple[str, str, str]]) -> int:
    ...
    manifest_text = read_required_text(
        artifact_root / "manifest.v03-story-promotion-live-review.txt", errors
    )
    report_text = read_required_text(
        artifact_root / "story-promotion-live-review-report.txt", errors
    )
    ...
    for path, fingerprint_path in [
        (
            artifact_root / "manifest.v03-story-promotion-live-review.txt",
            artifact_root / "manifest.fingerprint.txt",
        ),
        (
            artifact_root / "story-promotion-live-review-report.txt",
            artifact_root / "story-promotion-live-review-report.fingerprint.txt",
        ),
    ]:
    ...
    if "AIL-Story-Promotion-Live-Reviewer-Harness-Manifest:" not in manifest_text:
        errors.append(
            "manifest missing AIL-Story-Promotion-Live-Reviewer-Harness-Manifest header"
        )
    if "AIL-Story-Promotion-Live-Reviewer-Harness:" not in report_text:
        errors.append("report missing AIL-Story-Promotion-Live-Reviewer-Harness header")
    ...
    if errors:
        ...
        review_lines.append("repair-backlog story-promotion-live-review-repair-backlog.txt")
        review_lines.append(f"repair-backlog-fingerprint {nonaccept_backlog_fingerprint}")
    else:
        review_lines.append("review-result accepted")
    review_text = "\n".join(review_lines) + "\n"
    try:
        write_text(artifact_root / "story-promotion-live-review-review.txt", review_text)
        write_text(
            artifact_root / "story-promotion-live-review-review.fingerprint.txt",
            fnv64(review_text) + "\n",
        )
        if nonaccept_backlog_text:
            write_text(
                artifact_root / "story-promotion-live-review-repair-backlog.txt",
                nonaccept_backlog_text,
            )
            write_text(
                artifact_root / "story-promotion-live-review-repair-backlog.fingerprint.txt",
                nonaccept_backlog_fingerprint + "\n",
            )
    except OSError as error:
        print(review_text, end="")
        print(f"error failed to write Story Promotion live reviewer review report: {error}")
        return 1
    print(review_text, end="")
...
def run_live(args: argparse.Namespace, contracts: list[tuple[str, str, str]]) -> int:
    ...
    report_lines = [
        ...
        *token_budget_lines(args.max_tokens),
        ...
        f"story-artifacts {args.story_artifacts}",
    ...
    manifest_lines = ["AIL-Story-Promotion-Live-Reviewer-Harness-Manifest:"]
    ...
    for role, contract_path, contract_text in contracts:
        ...
        write_text(fingerprint_path_for(request_path), fnv64(request_text) + "\n")
        write_text(fingerprint_path_for(response_path), fnv64(response_text) + "\n")
        write_text(fingerprint_path_for(content_path), fnv64(content_text) + "\n")
        report_lines.append(
            f"role {role} contract {contract_path} "
            f"contract-fingerprint {fnv64(contract_text)} "
    ...
    report_text = "\n".join(report_lines) + "\n"
    manifest_text = "\n".join(manifest_lines) + "\n"
    write_text(artifact_root / "story-promotion-live-review-report.txt", report_text)
    write_text(
        artifact_root / "story-promotion-live-review-report.fingerprint.txt",
        fnv64(report_text) + "\n",
    )
    write_text(artifact_root / "manifest.v03-story-promotion-live-review.txt", manifest_text)
